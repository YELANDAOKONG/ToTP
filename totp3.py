...
def generate_totp(key: str, timestamp: int, window: int = 30000, token_long: int = 128) -> str:
    ...
    token = token.replace("=", "+")
    while True:
        if len(token) < token_long:
            token = token + token
        if len(token) > token_long:
            token = token[0:token_long]
            break
        if len(token) == token_long:
            break
    return token
...
def generate_totp_number(key: str, timestamp: int, window: int = 30000, token_long: int = 128) -> str:
    key_bytes = key.encode()
    timestep = int(int(timestamp) / window)
    hash_algorithms = [hashlib.sha256, hashlib.sha384, hashlib.sha512, hashlib.md5, hashlib.sha1, hashlib.sha224]
    macs = b""
    for hash_algorithm in hash_algorithms:
        ...
        macs += mac
    # encoded_macs = base64.b64encode(macs).rstrip(b"=")
    int_numbers = [int.from_bytes(macs[i:i + 1], 'big') for i in range(len(macs))]
    return_strs = ""
    for i in int_numbers:
        return_strs = return_strs + str(i)
    encoded_macs = return_strs
    offset = 0
    token = encoded_macs[offset:offset + token_long]
    while True:
        if len(token) < token_long:
            token = token + token
        if len(token) > token_long:
...
def generate_totp_number_plus(key: str, timestamp: int, window: int = 30000, token_long: int = 128) -> str:
    key_bytes = key.encode()
    timestep = int(int(timestamp) / window)
    hash_algorithms = [hashlib.sha256, hashlib.sha384, hashlib.sha512, hashlib.md5, hashlib.sha1, hashlib.sha224,
                       hashlib.sha3_256, hashlib.sha3_384, hashlib.sha3_512, hashlib.sha3_224]
...
def generate_totp_number_now(key: str, window: int = 30000, token_long: int = 128) -> str:
    timestamp = int(time.time() * 1000)
    return generate_totp_number(key, timestamp, window=window, token_long=token_long)


def generate_totp_number_plus_now(key: str, window: int = 30000, token_long: int = 128) -> str:
    timestamp = int(time.time() * 1000)
    return generate_totp_number_plus(key, timestamp, window=window, token_long=token_long)
...
def generate_totp_number_simple(key: str, timestamp: int, window: int = 30000, token_long: int = 128) -> str:
    key_bytes = key.encode()
    timestep = int(int(timestamp) / window)
    hash_algorithms = [hashlib.sha256(), hashlib.sha384(), hashlib.sha512()]
    macs = bytes()
...
def generate_totp_number_simple_now(key: str, window: int = 30000, token_long: int = 128) -> str:
    timestamp = int(time.time() * 1000)
    return generate_totp_number_simple(key, timestamp, window=window, token_long=token_long)
...
def generate_totp_number_simplest(key: str, timestamp: int, window: int = 30000, token_long: int = 128) -> str:
    key_bytes = key.encode()
    timestep = int(int(timestamp) / window)
    hash_algorithm = hashlib.sha512()
    macs = bytes()
...
def generate_totp_number_simplest_now(key: str, window: int = 30000, token_long: int = 128) -> str:
    timestamp = int(time.time() * 1000)
    return generate_totp_number_simplest(key, timestamp, window=window, token_long=token_long)
